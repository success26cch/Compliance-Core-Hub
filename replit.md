...
-   **Modular Platform Design:**
    ...
    -   **Training Evidence File Library:** Each employee competency record in the Training & Awareness module now supports multi-file evidence uploads (drag-and-drop or file picker). Stores files as base64 in the `training_evidence_files` table (10 document categories: Certificate, Sign-Off Sheet, Work Instruction, Inspection Instruction, Calibration Procedure, SOP, Test Result, Photo, Video (reserved), Other). Controlled-document categories (WI, SOP, Inspection, Cal) expose extra fields: Document Number, Revision, and Operation/Process Name — exactly the fields production and inspection training records require. Files are viewable and downloadable inline, with per-record file count badges and expand/collapse panels. RLS-isolated per tenant. Designed for future video module integration.
...
2. **Database-layer RLS (added):** PostgreSQL Row-Level Security policies enforced on all 87 user-scoped tables. Even if a route bug omits a `WHERE user_id=?` clause, PostgreSQL returns zero rows for the wrong tenant.
