...
- **Post-Purchase Corey Welcome Flow:** `/welcome-corey` page (`client/src/pages/WelcomeCorey.tsx`) greets new Corey subscribers by name, shows platform-specific PWA install instructions (iOS Share→Add to Home Screen, Android prompt or manual ⋮ menu, desktop link), and 3 quick-start tips. When Stripe returns, `corey_pro` checkout success in `Settings.tsx` redirects here instead of staying on settings. On first visit to `/corey` as a Pro subscriber, a one-time welcome modal (Dialog) appears with the same install instructions — dismissed via localStorage flag `"corey-welcomed"`.
