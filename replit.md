...
-   **Preventive Maintenance Module** (`pmEquipment` + `pmRecords` tables): Equipment register with PM schedule tracking, frequency settings (daily/weekly/monthly/quarterly/semi-annual/annual/custom), overdue/due-soon/on-track dashboard summary, expandable equipment cards with recent PM history, "Log PM" quick-action per equipment, PM Log tab with equipment filter, auto-calculated next-due-date from frequency, PM procedure checklist displayed in record form, record detail dialog with corrective action callout. ISOManager sidebar: active `Maintenance` nav button (replaced "Coming Soon" placeholder). API: `/api/pm/equipment` and `/api/pm/records`.
