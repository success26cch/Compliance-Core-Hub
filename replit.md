...
## Features
- **Corey (Ask Corey)**: AI-powered chat acting as a Senior Occupational Health & Safety Compliance Expert and Lead ISO Auditor, referencing OSHA 29 CFR, DOT 49 CFR Part 40, and comprehensive ISO standards (9001, 14001, 45001, IATF 16949, AS9100, ISO 13485, ISO 22000, ISO 27001, ISO 50001, ISO 22301, ISO 31000, ISO 19011). Generates 8 OSHA/DOT compliance document templates (Drug & Alcohol Policy, OSHA Recordkeeping SOP, Respiratory Protection, Hearing Conservation, HazCom, Return-to-Duty Checklist, Incident Investigation, LOTO). **ISO/management system implementation documents are NOT generated as templates** — Corey explains the framework and requirements, then redirects users to ACSI (acsi-quality.com) for customized implementation. CCH is a DBA of ACSI.
...
- **Corey Team Seats**: Multi-seat team billing system. Admin creates team, purchases X seats at $99/seat/month (single Stripe subscription), invites team members by email. Each member gets private Corey access with isolated conversations. Tables: `corey_teams`, `corey_team_members`. API: `/api/team`, `/api/team/members`, `/api/team/seats`, `/api/team/join/:token`, `/api/team/checkout`. UI in Settings page.
...
- **/clinic-letter**: Free Employer Clinic Communication Letter generator. Creates injury-specific PDF letters for occupational health providers requesting first-aid-level treatment per 29 CFR 1904.7(a). Covers 9 injury types (laceration, strain/sprain, contusion, burn, eye injury, back injury, respiratory, skin condition, general). Includes first-aid treatment preferences, recordability triggers, clinical notes, and full 29 CFR 1904.7(a) first-aid summary. Linked from OSHA Recordkeeping course card on Training page. API: POST `/api/clinic-letter/generate`, GET `/api/clinic-letter/injury-types`. Generator: `server/generateClinicLetter.ts`.
